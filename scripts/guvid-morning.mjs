 ...
 * 1. Capture morning snapshot: net liq + VIX (via DxLink $VIX.X)
 * 2. Scan SPX + QQQ with 3 profiles (conservative / neutral / aggressive)
 * 3. Save snapshot to guvid-agent/daily/dates/{YYYY-MM-DD}
 * 4. Save scan results to guvid-agent/scans/dates/{YYYY-MM-DD}
 * 5. Save best ICs as positions to guvid-agent/positions/items/{auto-id}
 */
...
// ── WebSocket patch (ws + optional proxy) ─────────────────────────────────────
...
// ── TastyTrade credentials — ONLY Catalin's account ──────────────────────────
// HARD RULE: never load credentials from any other user's brokerAccounts.
...
async function getAccessToken() {
  const { cs, rt } = await fetchCredentials();
...
async function getMarketMetrics(accessToken, symbols) {
  const res = await axios.get(
    `${TASTY_BASE}/market-metrics?symbols=${symbols.join(',')}`,
    { ...axiosCfg, headers: { Authorization: accessToken } }
  );
  ...
  for (const item of items) {
    result[item.symbol] = {
      ivRank: round2(parseFloat(item['implied-volatility-index-rank'] || '0') * 100),
      iv:     round2(parseFloat(item['implied-volatility-index']      || '0') * 100),
    };
  }
  return result;
}
...
async function getNestedOptionChain(accessToken, symbol) {
  const res = await axios.get(
    `${TASTY_BASE}/option-chains/${encodeURIComponent(symbol)}/nested`,
    { ...axiosCfg, headers: { Authorization: accessToken } }
  );
...
async function getUnderlyingSpot(accessToken, symbol) {
  try {
    ...
    const res = await axios.get(
      `${TASTY_BASE}/market-data/by-type?${params.toString()}`,
      { ...axiosCfg, headers: { Authorization: accessToken } }
    );
...
function buildICs(expiration, profileName, profile, quotesMap, greeksMap) {
  ...
  for (let i = 0; i < Math.min(putCandidates.length, callCandidates.length); i++) {
    ...
    // credit = shortPut.mid + shortCall.mid - longPut.mid - longCall.mid
    ...
    // POP = 100 - max(shortPut absDelta, shortCall absDelta) * 100
...
async function main() {
  ...
  console.log(`Date: ${TODAY}  |  Run: ${new Date().toISOString()}`);
  ...
  // ── Step 2: Account + Morning Net Liq ────────────────────────────────────
  ...
  const morningNetLiq = round2(balances.netLiquidity);
  console.log(`  Morning Net Liq: $${morningNetLiq}`);

  // ── Step 3: Market metrics (IV Rank) ─────────────────────────────────────
  console.log('\n[3] Fetching market metrics (IV Rank)...');
  const metrics = await getMarketMetrics(accessToken, ['SPX', 'QQQ', 'SPY']);
  console.log(`  SPX IV Rank: ${metrics['SPX']?.ivRank ?? 'N/A'}%`);
  console.log(`  QQQ IV Rank: ${metrics['QQQ']?.ivRank ?? 'N/A'}%`);
  // Use SPY IV as VIX proxy from REST (fallback if DxLink $VIX.X doesn't arrive)
  const vixRest = metrics['SPY']?.iv ?? null;
  console.log(`  VIX (SPY IV proxy): ${vixRest ?? 'N/A'}`);

  // ── Step 4: DxLink quote token ────────────────────────────────────────────
  console.log('\n[4] Getting DxLink quote token...');
  ...
  // ── Step 6: Collect all symbols (options + $VIX.X) ───────────────────────
  ...
  console.log(`\n[6] Subscribing ${symArray.length} symbols via DxLink (incl. $VIX.X)...`);
  ...
  try {
    ...
    // ── VIX from DxLink (fall back to REST proxy if not received) ───────────
    let morningVix = null;
    const vixQuote = quotesMap['$VIX.X'];
    if (vixQuote) {
      const vixMid = mid(vixQuote.bidPrice, vixQuote.askPrice);
      if (vixMid && vixMid > 0) morningVix = round2(vixMid);
    }
    if (morningVix == null) {
      morningVix = vixRest;
      console.log(`  $VIX.X not received via DxLink — using REST proxy: ${morningVix ?? 'N/A'}`);
    } else {
      console.log(`  $VIX.X (DxLink): ${morningVix}`);
    }

    // ── Step 4 (from spec): Save morning snapshot ────────────────────────────
    ...
    console.log(`  Saved → guvid-agent/daily/dates/${TODAY}  (netLiq=$${morningNetLiq}, vix=${morningVix})`);

    // ── Step 5 (from spec): Build Iron Condors ───────────────────────────────
    ...
    // ── Step 6 (from spec): Save scan to Firestore ───────────────────────────
    console.log('\n[9] Saving scan results to Firestore...');
      ...
      for (const [pName, ics] of Object.entries(results[ticker])) {
        for (const ic of ics) {
          ...
          batch.set(ref, {
            ...
            dailyChecks: [],
            marketContext: {
              underlyingPrice: spotPrices[ticker] ?? null,
              vix: morningVix,
              ivRank: metrics[ticker]?.ivRank ?? null,
            },
          });
          posCount++;
        }
    ...
    console.log('');
    for (const ticker of tickers) {
      const ivr = metrics[ticker]?.ivRank ?? 'N/A';
      const spot = spotPrices[ticker] ?? 'N/A';
      console.log(`${ticker}  spot=${spot}  IV Rank=${ivr}%`);
      for (const pName of Object.keys(PROFILES)) {
        const ics = results[ticker][pName];
        if (!ics.length) {
          console.log(`  ${pName.padEnd(12)}: — no qualifying ICs`);
        } else {
          ics.forEach(ic => console.log(
            `  ${pName.padEnd(12)}: DTE${ic.dte} ${ic.expiration}  credit=$${ic.credit}  RR=${ic.rr}  POP=${ic.pop}%  EV=${ic.ev}  score=${ic.score}`
          ));
        }
      }
    }
