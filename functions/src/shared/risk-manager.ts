...
${bpePercentage !== undefined ? `- Account BPE used: ${bpePercentage.toFixed(1)}% of paper account equity` : ''}
...
Review Picker's choice. Output JSON verdict.`;
    ...
    try {
        const result = await callClaude(PAPER_RISK_SYSTEM_PROMPT, userPrompt, {
            uid,
