...
async function main() {
  ...
  // Read morning doc — use legacy flat-doc path (guvid-agent/daily-YYYY-MM-DD)
  const dailyDocRef = db.doc(`guvid-agent/daily-${TODAY}`);
  ...
  // Read yesterday's doc for day-over-day (legacy flat-doc path)
  const yesterdayDocRef = db.doc(`guvid-agent/daily-${YESTERDAY}`);
  ...
  console.log(`  Saved to guvid-agent/daily-${TODAY}`);
